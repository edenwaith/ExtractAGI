				...
				if (signature == 0x1234) {
					...
					for (int i = 0; i < reslen; i++) {
						...
						if (byteVal >= 240) { // Action values are from 240 - 255
                            ...
                            NSLog(@"New Action:: %d: %@", selectedAction, [actions objectForKey: [NSNumber numberWithInt: selectedAction]]);
							...
							// 240: Change picture color
                        	if (selectedAction == 240) {
                            	pictureColor = colorPalette[byteVal];
                        	}

                        	// 242: Change priority color
                       		else if (selectedAction == 242) {
                            	priorityColor = colorPalette[byteVal];
                            }

							// 244 + 245: Corner drawing (244 - y corner, 245 - x corner)
